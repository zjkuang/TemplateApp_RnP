...
class RESTfulModel {
    ...
    class func save(data: Data, onCompletion: @escaping (String?, StorageErrorInfo) -> ()) {
        ...
        else { // real RESTful API
            
            // call real RESTful API to save the data, suppose RESTful API returns the index string for future retrieve
    ...
    class func retrieve(index: String, onCompletion: @escaping (Data?, StorageErrorInfo) -> ()) {
        ...
        if simulate {
            ...
            // call real RESTful API to retrieve the data
            let data = Data()
            let successful = true
